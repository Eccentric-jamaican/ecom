...
export default function ProductDetailsPage() {
  ...
  const fetchItem = useAction(api.products.getItem);
  ...
  const showPlaceholderDetails = !product && !error;
  const displayHeading = product?.title ?? (error ? "Listing unavailable" : "Loading listing...");
  const displayImage =
    product?.imageUrl ??
    "https://lh3.googleusercontent.com/aida-public/AB6AXuAUVgQRBadvnR47Ltj8nNYcl0BluMB8vg5XOftQVdq_hnFtpaagw1kTXGK_O3bWvrxX2NgxN_fgKmtmb29knYkyct-zhZCdHTsLIG3Neua2U5bmgjk7XoeZ4apf8Tv-cRtk9IWvNC9VjVE8Xsl7kG9rULIgDZH5XrMybmZbMk0LutPnN6dKPvg2VJXLUDyLn9dnq_2qwSAFczvk49w5x-mT_vUbMglUiq8snbiZOO6rumYdoxcufrLQ66WFUXyyS7YQyKdhcC9x0nE";
  const displayCondition = product?.condition ?? "Condition not listed";
  const displaySeller = product?.seller?.name;
  const displayPrice = useMemo(() => {
    if (!product) return "$129.00";
  ...
  const description = product
    ? `Live listing from eBay. ${displayCondition ? `Condition: ${displayCondition}. ` : ""}${displaySeller ? `Seller: ${displaySeller}.` : ""}`
    : error
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-4 md:px-10 py-6 md:py-10">
      ...
      <div className="flex flex-wrap gap-2 pb-6 md:pb-10 text-sm">
        ...
        <Link className="text-[#897561] font-medium leading-normal hover:text-primary hover:underline" href="/?category=all">Shop</Link>
        <span className="text-[#897561] font-medium leading-normal">/</span>
        <span className="text-[#181411] dark:text-white font-medium leading-normal">{displayHeading}</span>
      </div>
        ...
        <div className="lg:col-span-7 flex flex-col-reverse lg:flex-row gap-4">
          ...
          <div className="flex lg:flex-col gap-3 overflow-x-auto lg:overflow-y-auto scrollbar-hide lg:max-h-[600px] min-w-[80px]">
            {galleryImages.map((image, index) => (
              <button
                key={image.src}
                className={`w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden border-2 bg-gray-100 ${
                  index === 0
              ...
              >
                <Image
                  ...
                  className={`w-full h-full object-cover ${
                    index === 0 ? "opacity-100" : "opacity-80 hover:opacity-100 transition-opacity"
                  }`}
                  alt={image.alt}
                  src={image.src}
                />
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="border-b border-[#f4f2f0] dark:border-[#332a22] pb-6">
            ...
            <div className="flex items-end gap-3">
              <h2 className="text-3xl font-bold text-[#181411] dark:text-white">{displayPrice}</h2>
              {showDiscount ? (
                <>
          ...
          {showPlaceholderDetails ? (
            <div className="space-y-6">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <Link
                  ...
                  className="flex-1 h-12 bg-primary hover:bg-[#d67e25] text-[#181411] font-bold rounded-lg shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 text-base"
                >
            ...
            <div className="space-y-4">
              <div className="rounded-lg border border-[#e6e0db] dark:border-[#443b32] bg-white dark:bg-[#1a1612] p-4 text-sm text-[#6b5c51] dark:text-[#a89b91]">
                <div className="flex flex-col gap-2">
                  <span>Condition: <span className="text-[#181411] dark:text-white font-medium">{displayCondition}</span></span>
                  {displaySeller ? (
                    <span>Seller: <span className="text-[#181411] dark:text-white font-medium">{displaySeller}</span></span>
                  ) : null}
                </div>
              </div>
